...
export class CancelReservationToolService implements LlmTool {
  // Env Variables
  private readonly CANCEL_URL = this.retrieveEnvironmentVariablesService.retrieve<string>('LIBCAL_CANCEL_URL');
    ...
    try {
      ...
      if (response.success) {
        return `Room reservation with ID: ${bookingID} is cancelled successfully\n`;
          ...
          return `Room reservation with ID: ${bookingID} is notcancelled unsuccessfully. Error message: ${response.error}\n`;
      }
    } catch (error: any) {
