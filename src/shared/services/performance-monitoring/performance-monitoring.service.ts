...
export class PerformanceMonitoringService {
   ...
   */
  getPerformanceStats(timeWindowMs: number = 3600000): PerformanceStats {
    // Default 1 hour
    const now = Date.now();
  ...
  checkRateLimit(
    ...
    if (!allowed) {
      this.logger.warn(
        `Rate limit exceeded for key: ${key} (${entry.count}/${config.maxRequests})`,
      );
    }
  ...
  getSlowestOperations(
    ...
    limit: number = 10,
  ): Array<{ operation: string; duration: number; timestamp: string }> {
    const now = Date.now();
    const cutoffTime = now - timeWindowMs;
  ...
  getFailedOperations(
    ...
    return this.metrics
      .filter(
        (metric) =>
          !metric.success && new Date(metric.timestamp).getTime() > cutoffTime,
      )
      .slice(-limit);
  }
